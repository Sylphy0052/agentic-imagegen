...
- **Anima系へ移った場合も、タグの実在確認だけは同じ `scripts/tagcheck.py` を共用する。**
  引くのはDanbooruのタグで、系統によらず同じ台帳を見るため
...
- **引いていない `post_count` を書かない。** スクリプトを走らせていないタグへ「0件」や
  具体的な件数を添えない。実行していないなら「未確認」と書く。既存のpresetの中身へ
  言及するときも同じで、数値を出すなら先に引く
- 一度引いたタグは `.cache/tagcheck.json` へ30日残る。同じプロンプトを直しながら
  何度も確認してよい。サマリの `(キャッシュ N件)` がその件数。
  件数を取り直したいときだけ `--refresh` を付ける
