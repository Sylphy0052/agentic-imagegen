...
class TestExpandSeeds:
    def test_without_seeds_keeps_spec_as_is(self, tmp_path: Path) -> None:
        items = expand_seeds([("a.yaml", _spec())], seeds=None)
    ...
    def test_expands_each_spec_per_seed(self) -> None:
        items = expand_seeds([("a.yaml", _spec())], seeds=[1, 2, 3])
    ...
    def test_keeps_other_fields(self) -> None:
        items = expand_seeds([("a.yaml", _spec())], seeds=[7])
    ...
    def test_multiple_specs_are_expanded(self) -> None:
        pairs = [("a.yaml", _spec()), ("b.yaml", _spec())]
...
class TestRunBatch:
    async def test_runs_all_items(self, tmp_path: Path) -> None:
        items = [
            BatchItem(source="a.yaml", spec=_spec(), seed_override=None),
            BatchItem(source="b.yaml", spec=_spec(), seed_override=None),
        ]
        calls: list[str] = []
    ...
    async def test_continues_after_failure(self, tmp_path: Path) -> None:
        ...
        items = [
            BatchItem(source="a.yaml", spec=_spec(), seed_override=None),
            BatchItem(source="b.yaml", spec=_spec(), seed_override=None),
            BatchItem(source="c.yaml", spec=_spec(), seed_override=None),
        ]

        async def runner(item: BatchItem) -> GenerationResult:
            if item.source == "b.yaml":
    ...
    async def test_records_error_per_item(self, tmp_path: Path) -> None:
        items = [BatchItem(source="a.yaml", spec=_spec(), seed_override=None)]
...
def test_batch_item_label() -> None:
    ...
    plain = BatchItem(source="specs/a.yaml", spec=_spec(), seed_override=None)
    swept = BatchItem(source="specs/a.yaml", spec=_spec(), seed_override=42)
