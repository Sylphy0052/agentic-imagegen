...
| キー | 型 | 既定値 | 値域 |
| --- | --- | --- | --- |
| `content` | string または セグメントの並び | 必須 | 下記参照 |
...
`content`は文字列、またはセグメント (`text` / `tate_chu_yoko` / `ruby`) の並びで指定する。
文字列指定は従来どおり横書き・縦書きとも1文字ずつ配置する。セグメントの並びは
縦中横 (`tate_chu_yoko: true`。縦書き中で数文字を横に組んで1セルへ収める組版、
「令和7年」の「7」など) とルビ (`ruby`。親文字の右へ添える小さい文字) を
使いたいときだけ使う書き方で、どちらも指定しないセグメントは文字列指定と
完全に同じ結果になる。青空文庫式`｜漢字《かんじ》`のような
区切り記号によるインライン記法は採らない (約物回転の対象文字と衝突するため)。

| キー (セグメント) | 型 | 既定値 | 値域 |
| --- | --- | --- | --- |
| `text` | string | 必須 | 1文字以上。改行のみ制御文字として許可 |
| `tate_chu_yoko` | bool | `false` | `true`のセルは改行を含められず、1-4文字まで |
| `ruby` | string | `null` | 1-20文字。改行は不可。指定した場合は`text`にも改行を含められない |

- 全セグメントの`text`を合計した文字数は`content`の上限(500文字)と同じ規則で数える。
  `ruby`はこの合計に含めず、セグメントごとの上限(20文字)で抑える
- `direction: horizontal`では`tate_chu_yoko: true`も`ruby`も指定できない
  (横書きに縦中横の概念が無く、ルビの実装も縦書き専用のため)
- `max_width`で折り返す場合、縦中横のセルもルビ付きの親文字も1セルとして扱い、
  文字の途中では折り返さない
- セルの幅に収まらない場合は横方向へ縮小して1セルへ収める。
  縦中横を使っても行の高さと列の幅は変わらない
...
- 文字サイズは親文字の0.5倍。列の右側に確保したルビ帯へ縦に並べる
- レイヤ内に1つでもルビがあれば、**ルビの付かない列も含めて全列の幅**を
  ルビ帯のぶん広げる (列の間隔が揃わないと縦組みとして読みにくくなるため)
- 親文字よりルビが短い場合は親文字の全長へ均等割りする
- 親文字よりルビが長い場合は詰めずに親文字の前後へはみ出す。
  同じ列で隣り合うセルにもルビがあると、はみ出したぶんが重なることがある
- 親文字の字送りはルビの有無で変わらない。列の高さも変わらない
