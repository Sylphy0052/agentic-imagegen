...
`content`は文字列、またはセグメント (`text` / `tate_chu_yoko`) の並びで指定する。
文字列指定は従来どおり横書き・縦書きとも1文字ずつ配置する。セグメントの並びは
縦中横 (`tate_chu_yoko: true`。縦書き中で数文字を横に組んで1セルへ収める組版、
「令和7年」の「7」など) を使いたいときだけ使う書き方で、`tate_chu_yoko`を指定しない
セグメントは文字列指定と完全に同じ結果になる。青空文庫式`｜漢字《かんじ》`のような
...
- `direction: horizontal`では`tate_chu_yoko: true`を指定できない (横書きに縦中横の概念が無いため)
- `max_width`で折り返す場合、縦中横のセルは1セルとして扱い、文字の途中では折り返さない
